...
namespace Mag3DView.Nzy3dAPI.Plot3D.Rendering.Views
{
	...
	public class Camera : AbstractViewport
	{
        ...
        public Matrix4 GetViewMatrix()
        {
            ...
            var rotationMatrixY = Matrix4.CreateRotationY(MathHelper.DegreesToRadians(RotationY));
            var view = Matrix4.LookAt(new Vector3(2, 2, -Zoom), Vector3.Zero, Vector3.UnitY);
            return rotationMatrixX * rotationMatrixY * view;
        }

        public Matrix4 GetProjectionMatrix(float aspectRatio)
        {
            return Matrix4.CreatePerspectiveFieldOfView(
                MathHelper.DegreesToRadians(45f / Zoom),
                aspectRatio,
                0.1f,
                100f);
        }
        ...
        public Camera()
        {
            Position = new Coord3d(0, 0, 5); // Default position
            Target = new Coord3d(0, 0, 0);   // Default target
            Up = new Coord3d(0, 1, 0);       // Default up vector
        }

        public Camera(Coord3d targetPosition)
        {
            Position = new Coord3d(0, 0, 5); // Default position
            Target = targetPosition ?? new Coord3d(0, 0, 0); // Ensure Target is not null
            Up = new Coord3d(0, 1, 0);      // Default up vector
        }
        ...
        public void Rotate(double angleX, double angleY)
        {
            ...
            double azimuth = _eye.Azimuth + angleX;
